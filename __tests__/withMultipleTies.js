...
describe('Rankings with Multiple Ties', () => {
  ...
  describe('Ascending Rankings', () => {
    test('Returns an array of 5 objects in ascending order with multiple ties', () => {
      const rankings = ranky(baseOpts);

      expect(rankings).toHaveLength(5);
      expect(rankings).toEqual(expect.arrayContaining(expected));
    });
  });
});
